function [OmpResult] = OMP(y,A,S)
% #########################################################################
% Orthogonal Matching Pursuit Algorithm for Sparse Signal Recovery --- OMP
% [OmpResult] = OMP(y,A,S) solves the underdetermined system y=Ax where
...
for k = 1:S
    [~,lambda] = max(r'*A(:,1:Ns)) ;
    LAMBDA(k)=lambda ;
    Alm = A(:,LAMBDA(1:k)) ;  % the support set
    tilde_theta=(Alm'*Alm)\(Alm'*y);
    r = y- Alm*tilde_theta; %residual error
end


OmpResult = zeros(Ns,1) ;
OmpResult(LAMBDA) = tilde_theta ;
